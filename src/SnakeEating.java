...
class SnakeEating {
    ...
    // find postion that enough snakes can be eaten to grow snake from start to pos to length k
    int binarySnake(int start, int pos, int step, int k)
    {
        //out.println("pos "+pos+" step="+step);
        int next=pos+step;
        long diff=prefix[next];
        if (start>0)
            diff -= prefix[start-1];
        diff = k*(next-start+1)-diff;
        if (diff>L.length-next-1) {
            if (step==0)
                return 0;
            return binarySnake(start, pos, step/2, k);
        }
        else if (diff==L.length-next-1)
            return next-start+1;
        else {
            if (step==0)
                return 1;
            return binarySnake(start, next, step/2, k);            
        }
    }
    int query(int k)
    {
        ...
        if (x>=L.length-1)  // edge test case
            return count;
        int c=binarySnake(x, x, (L.length-x)/2, k);
    ...
    static void test()
    {
        ...
        out.println(sn.query(10));
        out.println(sn.query(15));
        ...
        sn = new SnakeEating(new Integer[]{10, 15}, 3);
        out.println(sn.query(15));  // 1
        out.println(sn.query(16));  // 1
        out.println(sn.query(10));  // 2
        out.println(sn.query(17));  // 0
        */
        sn = new SnakeEating(new Integer[]{1000000000, 1500000000}, 3);
        out.println(sn.query(1000000000));  // 2
        out.println(sn.query(1500000000));  // 1
        out.println(sn.query(1500000001));  // 1
        out.println(sn.query(1500000002));  // 0
        out.println(sn.query(1500000003));  // 0
    }
